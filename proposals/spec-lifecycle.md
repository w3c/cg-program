...
* The status information is surfaced in the specification (at least).
   ...
   * for live documents, by default it is the one set of a time of publication, but we expect there will be a (JavaScript) mechanism that allows to mark documents as abandonned/transfered via data under W3C's control, to be used when the live doc does not reflect the real status of the spec and can't be otherwise updated (e.g. noone active with write access to the repo)
